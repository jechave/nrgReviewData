...
p1 <- ggplot(corTable, aes(y=cor.RSA, x=-cor.WCN)) + 
  ...
  ylim(0.3, 0.78) +
  coord_fixed() +
  xlab("Absolute Rate–WCN correlation") +
...
for(x in pdblist)
{
  ...
  write.table(scale(df_structure$K), paste("structures/", x, "-rate.txt", sep=''), row.names=F, col.names=F)
  write.table(df_structure$RSA, paste("structures/", x, "-RSA.txt", sep=''), row.names=F, col.names=F)
  write.table(scale(-1*df_structure$WCN), paste("structures/", x, "-WCN.txt", sep=''), row.names=F, col.names=F)
